...
const Add = ({ token }) => {
  ...
  return (
    <form onSubmit={onSubmit} className="flex flex-col w-full items-start gap-4">
      {/* Image upload */}
      <div>
          ...
          {images.map((img, i) => (
            <label key={i} htmlFor={`image${i}`} className="cursor-pointer">
              <img
                ...
                src={img ? URL.createObjectURL(img) : adminAssets.upload_area}
                alt={`Upload ${i + 1}`}
              />
              <input
                id={`image${i}`}
      ...
      <div className="w-full">
        ...
        <input
          ...
          className="w-full max-w-125 px-3 py-2 border border-gray-300 rounded outline-none focus:border-gray-500"
          type="text"
      ...
      <div className="w-full">
        ...
        <textarea
          ...
          className="w-full max-w-125 px-3 py-2 border border-gray-300 rounded outline-none focus:border-gray-500 resize-none"
          rows={4}
